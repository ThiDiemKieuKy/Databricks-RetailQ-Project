...
@dp.table(
    name=f"{silver_schema}.opportunity",
...
def opportunity_clean():
    ...
    source_df = spark.readStream.table(f"{bronze_schema}.opportunity")
    ...
    return source_df.select(
        ...
        F.when(F.col("amount") < 100000, "SMALL")
         .when(F.col("amount") < 25000, "ENTERPRISE")
         .otherwise("MID_MARKET")
         .alias("deal_size"),
